...
def convert_data_to_annotation(data, meta):
...
def process_lyft_scene(lyft, scene):
    ...
    dataset_data = []
    for i in tqdm(range(0, 70)):  # because pass first sample
        ...
        for ann_token in annotations:
            ann = lyft.get('sample_annotation', ann_token)
        ...
        data['instances'] = instance_tokens
...
class Superviselyft:
    ...
    def convert_scene_id(self, idx):
        ...
        instances = sum([x['instances'] for x in dataset_data], [])
        labels = np.concatenate([x['names'] for x in dataset_data]).tolist()

        unique_objs = {}
        for i in set(instances):
            label = labels[instances.index(i)]
            unique_objs[i] = sly.PointcloudObject(self.project_fs.meta.get_obj_class(label))
        sly.logger.info(f"{len(unique_objs)} unique object tracks")
        for data in dataset_data:
            ...
            figures = []
            obj_instances = []
            for l, geometry, token in zip(label, geometries, data['instances']):
                current_obj = unique_objs[token]
                figures.append(sly.PointcloudFigure(current_obj, geometry))
                obj_instances.append(current_obj)

            ann = sly.PointcloudAnnotation(PointcloudObjectCollection(obj_instances), figures)
            ...
            for sensor, image_path in data.items():
                if 'CAM' in sensor and not sensor.endswith('_intrinsic')\
                    ...
                    img_info = {
                        ...
                        "meta": {
                            "deviceId": sensor,
                            "timestamp": datetime.fromtimestamp(data['timestamp'] / 1000000).isoformat(),
                            "sensorsData": {
