...
def init(data, state):
    ...
    #data["projectPreviewUrl"] = g.api.image.preview_url(g.project_info.reference_image_url, 100, 100) # TODO: make it
...
def download(api: sly.Api, task_id, context, state, app_logger):
    try:
        if sly.fs.dir_exists(g.project_dir):
            ...
            reset_progress(progress_index) # TODO: make progress bar
